...
For RQ1, where we evaluate tools for constructing test method histories, we selected all 40 projects used in the most recent study, \emph{HistoryFinder}~\cite{islam_historyfinder_2025}. These 40 projects subsume the 20 projects previously used to evaluate both \emph{CodeShovel}~\cite{grund:2021} and \emph{CodeTracker}~\cite{jodavi_accurate_2022}. For RQ2, where we constructed the ground truth for production-to-test mappings, the annotation effort was substantially greater than for RQ1. To keep the manual effort manageable, we used the 20 projects used in \emph{CodeShovel} and \emph{CodeTracker} studies, rather than all 40 projects from \emph{HistoryFinder}. Despite this reduction, our benchmark is considerably larger than those used in prior work. For example, the state-of-the-art \emph{TestLinker}~\cite{sun_method-level_2024} was evaluated on only six projects. For RQ3 and RQ4, which rely exclusively on quantitative analysis, we combined all projects used in recent method-level evolution studies~\cite{grund:2021,islam_historyfinder_2025,chowdhury2024method,friesen_repeat_2025}, resulting in a dataset of 110 projects.    
...
To identify all the production and test files, we initially used the standard test and production source-root conventions. For example, in both Maven and Gradle Java projects, the default production and test source roots are \texttt{src/main/java} and \texttt{src/test/java}, respectively. Since projects can override or extend these defaults, we also analyzed build configuration files to identify project-specific source-root declarations, such as the \texttt{sourceDirectory} and \texttt{testSourceDirectory} elements in Maven \texttt{pom.xml} files and the \texttt{sourceSets} properties in Gradle build files. Finally, we manually inspected each project's layout and found that some projects place test files under the default production source root, especially when an entire module is intended for testing. In these cases, we considered the corresponding files as test files. 

After identifying the test files, we classified methods within the file as either test methods or test-helper methods. Following prior work that detects JUnit tests using framework-specific annotations and naming conventions~\cite{zaidman2011studying,tufano2022methods2test}, we first identified methods annotated with test annotations. These include, for example, \texttt{org.junit.jupiter.api.Test} for JUnit, \texttt{org.testng.annotations.Test} for TestNG, and \texttt{net.jqwik.api.Property} for jqwik. To detect pre-JUnit 4.x tests, we identified public, parameterless \texttt{void} methods whose names begin with \texttt{test} as test methods when their declaring class extends \texttt{junit.framework.TestCase} or one of its descendants. All other methods declared in test files are classified as test-helper methods---methods that are not directly testing any production methods. This identification was important as these methods were filtered out during production-to-test mapping.  

We labelled other files as production that were not identified as test, and extracted all methods as production methods. Similar to prior works~\cite{Heitlager:2007, Fontana:2015, chowdhury2022empirical}, we filtered out \texttt{getters} and \texttt{setters}; these methods are often auto-generated and create noise in software maintenance research. 