...
Our previous analysis shows that revisions to test methods are not necessarily driven solely by changes in the production methods they exercise. Test methods may also evolve because of internal maintainability issues. Test smells provide a useful lens for examining this possibility, as they capture potentially problematic characteristics of test code~\cite{bavota2015test,Spadini:2018,panichella2022test}. Prior work has shown that test smells are most often introduced in the initial version of a test method~\cite{tufano2016empirical}. Therefore, we investigate whether the presence of test smells in the first version of a test method is associated with a high number of future revisions.

\subsubsection{Approach}
For smell detections, we used \emph{JNose}~\cite{virginio2020jnose}. Using rule-based detectors derived from established test-smell definitions and detection rules from prior work~\cite{tufano2016empirical,garousi2018smells}, \emph{JNose} detects different types of test smells and provides method-level smell locations, making it appropriate for our study. To understand the association of test smells with method evolution, we divide the test methods into three groups and label them for easy reference for subsequent analysis. 
\begin{itemize}
...
is used. To reduce ambiguity, we filtered such methods out for this RQ. Also, to ensure
sufficient project-level coverage, we included projects for which the strategy establishes at least 30 links. After applying this threshold, the analysis includes 71,231 \emph{NTR} methods from 90 projects, 23,494 \emph{MTR} methods from 88 projects, and 3,274 HTR methods from 74 projects. 

We used the following metrics for comparing these three groups of methods. 
\begin{itemize}
    \item \textbf{Percent (\%):} For each individual smell, we calculated its prevalence in our three groups of methods. We also calculated the difference in percent (\textbf{$\Delta$ pp}). 
    \item \textbf{Odds ratio (OR):} Suppose, we are calculating the odds ratio by using \emph{NTR} and \emph{HTR} groups. Let $a$ be the number of HTR methods with a smell, $b$ the
...
that the smell is more prevalent among HTR methods. We report the conditional
maximum likelihood estimate of the odds ratio and its exact 95\% confidence
interval. 
\item  \textbf{Fisher's
exact tests ($p_{BH}$)~\cite{fisher1922interpretation}:} We also used two-sided Fisher's
exact tests because the group sizes are unequal and several smells are rare. We also applied the
Benjamini--Hochberg~\cite{benjamini1995controlling} procedure across the individual-smell comparisons to
...
As a sensitivity analysis, we additionally estimated a Mantel-Haenszel odds
ratio for each smell while treating projects as strata, and applied the same
...
% \small
% \input{figure/t2p-test-smell-association-table--historyFinder--omc--nc--jnose--ch_diff--MHTR-NTR}
% \end{table*}
...
% \centering
% \mbox{
% \subfigure[]{\includegraphics[width=0.38\textwidth,keepaspectratio]{figure/t2p-test-smell-size-control-effectplot--historyFinder--omc--nc--jnose--ch_diff--HTR-NTR-MTR-NTR}}
% \subfigure[]{\includegraphics[width=0.38\textwidth,keepaspectratio]{figure/t2p-test-smell-size-control-odds-ratio-effectplot--historyFinder--omc--nc--jnose--ch_diff--HTR-NTR-MTR-NTR}
% }
% }
...
% \small
% \input{figure/t2p-test-smell-association-table--historyFinder--omc--nc--jnose--ch_diff}
% \end{table*}
...
We also examined the potential confounding effect of method size when interpreting the relationship between test smells and method evolution. Prior studies have shown that many code metrics and smells lose their predictive power for software quality and maintainability once size is controlled for, as size is often strongly correlated with these metrics~\cite{Emam:2001,Gil:2017,TSE:2013}. For this analysis, we minimized noise by excluding smell types that did not exhibit a consistent statistically significant difference in Table~\ref{tab:rq4-ntr-mtr}. Specifically, we excluded \emph{Unknown Test}, \emph{Print Statement}, \emph{Redundant Assertion}, \emph{Lazy Test}, \emph{Empty Test}, and \emph{Ignored Test}. Using the remaining 11 smell types, we computed Kendall's $\tau$ correlation coefficient between size and the number of test smells per method. The resulting coefficient of 0.28 suggests that method size alone does not explain the observed relationship between test smells and method evolution. This apparent discrepancy with previous studies~\cite{Emam:2001,Gil:2017,TSE:2013} is unsurprising, as those studies primarily reported strong correlations between size and software metrics at the class or file level. In contrast, subsequent work has shown that such correlations are considerably weaker at the method level~\cite{Landman:2014,chowdhury:2022}, which is the granularity of our study.

To further investigate the impact of method size, following the methodology of Spadini \textit{et al.}~\cite{Spadini:2018}, we categorized test methods into three size groups: small ($size < 30$), medium ($30 \leq size \leq 60$), and large ($size > 60$). Figure~\ref{fig:size} presents the results of this size-stratified analysis. Consistent with our earlier findings, both the difference in percent (\textbf{$\Delta$ pp}) and the odds ratio are larger when comparing \emph{HTR} methods with \emph{NTR} methods than when comparing \emph{MTR} methods with \emph{NTR} methods. More importantly, the association between test smells and method evolution is strongest for small methods: both the percent difference and the odds ratios are considerably larger for small methods than for medium or large methods. This observation contradicts earlier findings~\cite{Spadini:2018} that test smells in large methods are more harmful than smells in smaller methods. 
...
% \footnotetext{\href{https://anonymous.4open.science/r/test-evolution/data/rqs/rq4/test-smell-difference.pdf}{/data/rqs/rq4/test-smell-difference.pdf}}
\begin{framed}
