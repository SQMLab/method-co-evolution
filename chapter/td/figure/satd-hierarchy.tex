...
\node {SATD in test code\\(\totalSatdCount)}
  ...
  child {node (test-debt) [fill=black!80, text=white, xshift=-5pt, inner xsep=6pt] {\addedB{Test}\\(\newTypeCountB)}
    % [level distance=18mm]
    % child {node[fill=black!80, text=white, xshift=-130pt] {Composite\\(\compositeCount)}}
    child {node (uncertainty-test) [fill=black!80, text=white, xshift=-65pt] {\addedB{Uncertainty}\\(\uncertaintyCountB)}}
    child {node[fill=black!80, text=white, xshift=-55pt] {\addedB{On-hold}\\(\onHoldCountB)}}
    child {node[fill=black!80, text=white, xshift=-50pt] {Skip test\\(\skipTestCountB)}}
    child {node (limited-test) [fill=black!80, text=white, xshift=-37pt] {\addedB{Limited test}\\(\limitedTestCountB)}}
  }
  child {node (other-debt) [xshift=-15pt] {\added{Other debt}\\(\otherCount)}};
