...
\node {SATD in test code\\(\totalSatdCount)}
  ...
  child {node (test-debt) [fill=black!80, text=white, xshift=-5pt, inner xsep=6pt] {Test\\(\newTypeCount)}
    ...
    child {node (uncertainty-test) [fill=black!80, text=white, xshift=-65pt] {Uncertainty\\(\uncertaintyCount)}}
    child {node[fill=black!80, text=white, xshift=-55pt] {On-hold\\(\onHoldCount)}}
    child {node[fill=black!80, text=white, xshift=-50pt] {Skip test\\(\skipTestCount)}}
    child {node (limited-test) [fill=black!80, text=white, xshift=-37pt] {Limited test\\(\limitedTestCount)}}
  }
