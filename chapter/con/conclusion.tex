...
Test-code evolution at the method level requires reliable evidence about how individual test methods change over time. Prior method-history tools developed for production methods can also be applied to test methods, with \textit{HistoryFinder} achieving the strongest overall performance.

Building on this method-history support, method-level history tracking is combined with production-to-test mapping to analyze how test methods evolve in relation to the production methods they exercise. The findings show that many test methods evolve as much as, or more than, their corresponding production methods. They also show that highly revised small test methods are more strongly associated with test smells. These results indicate that test methods can have their own evolution patterns, rather than serving only as passive reflections of production-code change.
