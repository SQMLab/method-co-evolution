...
def establish_confidence(row):
    ...
    return pd.Series({
        ...
        "tech_lcs_b": lcsBoth._compute_lcs_score(production_method_name, test_method_name),
        "tech_lcs_u": lcsUnit._compute_lcs_score(production_method_name, test_method_name),
        "tech_leven": ld._compute_levenshtein_score(production_method_name, test_method_name)})
...
for _, repo in repository_df.iterrows():
    ...
    if os.path.exists(fan_out_file):
        ...
        fan_out_df[["tech_nc", "tech_ncc", "tech_lcs_b", "tech_lcs_u", "tech_leven"]] = fan_out_df.apply(
        ...
        fan_out_df["tech_lcba"] = (
            fan_out_df.index.isin(
                fan_out_df[fan_out_df["to_lcba"] == 1]
                .groupby("from_url")
                .tail(1)
                .index
            )
        ...
        fan_in_count_file = f"{DATA_DIRECTORY}/m2m-tech/{repository_name}.csv"
